jQuery( function( $ ) {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					appendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$('.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			...
			getAllInputs($(this).parent().parent().parent());
		}
	});
	...
	function appendRow($element, defaultValue = '') {
		var container = $element.find('.sortable');
		var last_row  = container.find('.repeater:last');
		var new_row   = last_row.clone();
		new_row.appendTo( container ).show(function(){
			$(this).find('input').focus().val( defaultValue );
		});
	}
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.appthemesfontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.appthemesfontslist[index].variants, function(val, text) {
		...
		if(elementItalicWeightCount == 0) {
			elementItalicWeight.append(
				$('<option></option>').val('').html( elementItalicWeight.data( 'notAvailable' ) )
			);
			elementItalicWeight.prop('disabled', 'disabled');
		}
		if(elementBoldWeightCount == 0) {
			elementBoldWeight.append(
				$('<option></option>').val('').html( elementBoldWeight.data( 'notAvailable' ) )
			);
			elementBoldWeight.prop('disabled', 'disabled');
		}

		// Update the font category based on the selected font
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.appthemesfontslist[index].category);
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].appthemestinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].appthemestinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].appthemesmediabuttons;
