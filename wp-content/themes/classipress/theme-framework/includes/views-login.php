...
class APP_Login extends APP_Login_Base {
	...
	function process_form() {
		...
		if ( isset( $_GET['action'] ) && 'confirmaction' == $_GET['action'] ) {

			if ( ! isset( $_GET['request_id'] ) ) {
				wp_die( __( 'Missing request ID.', APP_TD ) );
			}

			if ( ! isset( $_GET['confirm_key'] ) ) {
				wp_die( __( 'Missing confirm key.', APP_TD ) );
			}
			...
			wp_die( $message, __( 'User action confirmed.', APP_TD ) );
		}
