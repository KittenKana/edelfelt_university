...
class APP_Google_Geocoder extends APP_Geocoder {
	...
	public function set_address_components() {
		...
		if ( empty( $this->geocode_results['results'][0]['address_components'] ) ) {
			return $output;
		}

		$data = (array) $this->geocode_results['results'][0]['address_components'];
	...
	public function parse_address_components( $data ) {
		...
		foreach ( $data as $key ) {
			...
			switch ( $key['types'][0] ) {
				...
				case 'street_number':
					$output['street_number'] = $key['long_name'];
					break;
				case 'route':
					$output['street'] = $key['long_name'];
					break;
				case 'sublocality_level_1':
				case 'locality':
				case 'postal_town':
					$output['city'] = $key['long_name'];
					break;
				case 'administrative_area_level_2':
					if ( empty( $output['city'] ) ) {
						$output['city'] = $key['long_name'];
					} else {
						$output['state_short'] = $key['short_name'];
						$output['state_long']  = $key['long_name'];
						$output['state']       = $key['long_name'];
					}
					break;
				case 'administrative_area_level_1':
					...
					break;
				case 'postal_code':
					$output['postal_code'] = $key['long_name'];
					break;
				case 'country':
					$output['country_short'] = $key['short_name'];
					$output['country_long']  = $key['long_name'];
					$output['country']       = $key['long_name'];
					break;
			}
		}
			...
			array(
				...
				'fields' => array(
					array(
						...
						'desc' => sprintf( __( 'Find your two-letter <a href="%s" target="_blank">region code</a>', APP_TD ), 'https://en.wikipedia.org/wiki/ISO_3166-1#Current_codes' ),
						'type' => 'text',
						'name' => 'geo_region',
					...
					array(
						...
						'desc' => sprintf( __( 'Find your two-letter <a href="%s" target="_blank">language code</a>', APP_TD ), 'https://en.wikipedia.org/wiki/List_of_ISO_639_language_codes' ),
						'type' => 'text',
						'name' => 'geo_language',
