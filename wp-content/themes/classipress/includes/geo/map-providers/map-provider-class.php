...
abstract class APP_Map_Provider {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'appthemes-maps', get_template_directory_uri() . '/includes/geo/map-providers/appthemes-maps.js', array( 'jquery-ui-widget' ), '20180916', true );
	...
	private function map_provider_vars() {
		...
		$global_options = APP_Map_Provider_Registry::get_options();
		$defaults       = array(
			'geo_unit'       => $global_options->geo_unit,
			'default_radius' => $global_options->default_radius,
			'no_geocode'     => __( "Autocomplete's returned place contains no geometry", APP_TD ),
		);

		$this->map_provider_vars = wp_parse_args( $this->map_provider_vars, $defaults );
