...
class APP_Dynamic_Checkout{
	...
	/**
	 * Flag on whether to do redirect to next step after finished one.
	 *
	 * @var bool
	 */
	...
	public function __construct( $checkout_type, $hash = '' ) {
		...
		if ( empty( $this->hash ) ) {
			...
			do_action( 'appthemes_checkout_created', $this );
			do_action( "appthemes_checkout_{$this->checkout_type}_created", $this );
		}
	...
	/**
	 * Sets redirect flag to checkout object.
	 *
	 * @param bool $do_redirect Whether to do redirect to next step after
	 *                           finished one.
	 */
	public function set_redirect_flag( $do_redirect = true ) {
		$this->redirect = (bool) $do_redirect;
	}
	...
	public function complete_checkout() {
		...
		if ( $this->get_data( 'completed' ) ) {
			return;
		}

		do_action( 'appthemes_checkout_completed', $this );
		do_action( "appthemes_checkout_{$this->get_checkout_type()}_completed", $this );
		$this->add_data( 'completed', 1 );
	...
	protected function call_step( $id, $type = 'display' ){
		...
		// Reset current step since it might be lost in process.
		$this->current_step = $id;
		...
		if ( $this->step_cancelled ) {
			$prev_step = $this->get_previous_step( $id );
			if ( $this->redirect ) {
				$this->redirect( appthemes_get_step_url( $prev_step ) );
				appthemes_exit( "cancel_step_{$this->current_step}" );
			} else {
				$this->step_cancelled = false;
				$this->process_step( $prev_step );
			}
		}
		...
		if ( $this->step_finished ) {
			...
			if ( ! $next_step ) {
				...
				$this->step_finished  = false;
				$this->process_step( $next_step );
			}
		}
