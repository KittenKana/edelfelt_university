...
class APP_PHPColors {
	 ...
	 *
	 * @param string $color The given color string in HEX or RGBA format.
	 ...
	 *
	 * @param array|string $rgb The color.
	...
	/**
	 * Checks the lightness of given color, and if it passes the `$threshold`
	 * of lightness, it returns the darker color. Otherwise, it returns the
	 * lighter color.
	 *
	 * Use this function to dynamically output a foreground color based on a
	 * given background color.
	 *
	 * @param int    $amount    The amount of lightness/darkness that needs to
	 *                          be  adjasted to a given color.
	 * @param string $threshold Threshold of lightness to check against.
	 * @return string
	 */
	public function foreground( $amount = self::DEFAULT_ADJUST, $threshold = 130 ) {

		if ( $this->isLight( false, $threshold ) ) {
			$color = $this->darken( $amount );
		} else {
			$color = $this->lighten( $amount );
		}
	 ...
	 * @return string
	 */
	public function smart_scale( $amount = self::DEFAULT_ADJUST, $threshold = 130 ) {
		if ( $this->isLight( false, $threshold ) ) {
			$color = $this->lighten( $amount );
		} else {
			$color = $this->darken( $amount );
	 ...
	 * @param string|Boolean $color The color.
	 ...
	 * @param string|Boolean $color The color string.
	...
	/**
	 * You need to check if you were given a good rgba string or array.
	 *
	 * @param string|array $rgb Given color.
	 * @return string Color
	 * @throws Exception 'Bad color format'.
	 */
	private static function _checkRgb( $rgb ) {
		...
		if ( 3 > count( (array) $rgb ) ) {
			throw new Exception( 'Param was not an RGB array' );
		}

		$values = array_values( $rgb );

		$rgb['R'] = $values[0];
		$rgb['G'] = $values[1];
		$rgb['B'] = $values[2];
		$rgb['A'] = ( isset( $values[3] ) ) ? $values[3] : 1;
	...
	public function __toString() {
		if ( $this->alpha < 1 ) {
			$color = sprintf( 'rgba(%s) ', implode( ',', $this->_rgb ) );
		} else {
			$color = '#' . $this->getHex();
		}
		return $color;
