...
class APP_Order_Processes_Test extends APP_UnitTestCase {
	...
	public function test_escrow_orders() {
		$user_id = $this->factory->user->create( array( 'role' => 'contributor' ) );
		$order   = APP_Escrow_Order_Factory::create();
		$order->add_item( 'test', 10 );
		$order->add_receiver( $user_id, 5, false );

		$order     = appthemes_get_order( $order->get_id() );
		$receivers = $order->get_receivers();
		$this->assertEquals( array( $user_id => 5 ), $receivers );
	}
