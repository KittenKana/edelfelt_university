...
class CP_Importer extends APP_Importer {
	...
	function setup() {
		...
		add_action( 'appthemes_importer_import_row_after', array( $this, 'import_row_after' ), 10, 2 );
	...
	public function import_row_after( $listing_id, $row ) {

		if ( $this->post_type != get_post_type( $listing_id ) ) {
			return;
		}

		$address_comps = array(
			'cp_street'  => ! empty ( $row['street'] ) ? $row['street'] : '',
			'cp_city'    => ! empty ( $row['city'] ) ? $row['city'] : '',
			'cp_state'   => ! empty ( $row['state'] ) ? $row['state'] : '',
			'cp_zipcode' => ! empty ( $row['zipcode'] ) ? $row['zipcode'] : '',
			'cp_country' => ! empty ( $row['country'] ) ? $row['country'] : '',
		);

		cp_geocode_address( $listing_id, $address_comps );
	}
