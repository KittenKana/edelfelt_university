...
if ( ! function_exists( 'cp_upgrade_all' ) ) :
	function cp_upgrade_all() {
		...
		if ( ! empty( $current_db_version ) && $current_db_version < 1320 ) {
			cp_upgrade_320();
		}
		...
		if ( $current_db_version < 2904 ) {
			cp_upgrade_400();
		}

		if ( $current_db_version < 2920 ) {
			cp_upgrade_favicon_400();
		}

		if ( $current_db_version < 2999 ) {
			cp_upgrade_410();
		}

		if ( ! empty( $current_db_version ) && $current_db_version < 3098 ) {
			cp_upgrade_420();
		}
		...
		foreach ( $post_ids as $post_id ) {
			if ( ! cp_get_geocode( $post_id ) ) {
				...
				if ( $address ) {
					...
					$geocode = json_decode( wp_remote_retrieve_body( wp_remote_get( 'https://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode( $address ) . '&sensor=false' ) ) );
...
/**
 * Execute changes made in ClassiPress 3.5.3.
 *
 * @since 3.5.3
 */
...
function cp_upgrade_400() {
	...
	$current_sidebar_user_widgets = array();

	if ( ! empty( $current_sidebars_widgets[ 'sidebar_user' ] ) ) {
		$current_sidebar_user_widgets = $current_sidebars_widgets[ 'sidebar_user' ];
	}
	...
	$layout_widgets = array(
		'sidebar_main_content' => array(
			...
			'widget-ad-categories' => array(
				...
				'sub_num'     => $cp_options->cat_dir_sub_num,
				'depth'       => $cp_options->cat_dir_depth,
				'cols'        => $cp_options->cat_dir_cols,
				'show_count'  => $cp_options->cat_dir_count,
				'hide_empty'  => $cp_options->cat_dir_hide_empty,
				'extra_class' => 'widget',
			),
	...
	appthemes_install_widgets( $layout_widgets );
		...
		'sidebar_listing' => array(
			...
			'text' => array(
				...
				'text'  => "<ul class='social-icons'>\n"
							...
							. "</li>\n"
							. "<li>\n"
							. "[classipress_share_button icon='fa-pinterest' share_url='https://pinterest.com/pin/create/button/?url=']\n"
							. "</li>\n"
						. "</ul>\n",
			),
	...
	foreach ( $existing_sidebars as $sidebar => $widgets ) {
		$widgets = array_reverse( $widgets );
		foreach ( $widgets as $widget_name => $instance_settings ) {
			appthemes_install_widget( $widget_name, $sidebar, $instance_settings, 1, 'prepend' );
		}
	}

	// Upgrade customizer options.
	set_theme_mod( 'color_scheme', str_replace( '.css', '', $cp_options->stylesheet ) );

	if ( '#EFEFEF' !== strtoupper( $cp_options->bgcolor ) ) {
		set_theme_mod( 'background_color', $cp_options->bgcolor );
	}

	if ( '#EFEFEF' !== strtoupper( $cp_options->header_bgcolor ) ) {
		set_theme_mod( 'header_background_color', $cp_options->header_bgcolor );
	}

	set_theme_mod( 'header_text_color', get_theme_mod( 'header_textcolor', '#8A8A8A' ) );
	set_theme_mod( 'footer_background_color', $cp_options->footer_bgcolor );
...
/**
 * Execute favicon changes made in ClassiPress 4.0.0.
 *
 * @since 4.0.0
 */
function cp_upgrade_favicon_400() {
	global $cp_options;

	// migrate favicon option to 'Site Icon feature'
	// https://codex.wordpress.org/Creating_a_Favicon#WordPress_Version_4.3_or_later
	$site_icon_id = get_option( 'site_icon' );

	if ( $site_icon_id ) {
		// reset our custom option
		$cp_options->favicon_url = '';
	} elseif ( $cp_options->favicon_url ) {
		cp_install_favicon( $cp_options->favicon_url );
	}
}

function cp_upgrade_410() {
	global $cp_options;

	$cp_options->geo_unit = $cp_options->distance_unit;
	$cp_options->geocoder_settings = array(
		'google' => array (
			'geo_region'   => $cp_options->gmaps_region ? $cp_options->gmaps_region : 'us',
			'geo_language' => $cp_options->gmaps_lang ? $cp_options->gmaps_lang : 'en',
			'geo_unit'     => $cp_options->distance_unit ? $cp_options->distance_unit : 'mi',
			'api_key'      => $cp_options->api_key,
		),
	);
	$cp_options->map_provider_settings = array(
		'google' => array (
			'geo_region'   => $cp_options->gmaps_region ? $cp_options->gmaps_region : 'us',
			'geo_language' => $cp_options->gmaps_lang ? $cp_options->gmaps_lang : 'en',
			'geo_unit'     => $cp_options->distance_unit ? $cp_options->distance_unit : 'mi',
...
function cp_upgrade_420() {
	...
	// Rename the theme mod.
	if ( get_theme_mod( 'header_text_color' ) ) {
		set_theme_mod( 'middle_header_menu_color', get_theme_mod( 'header_text_color' ) );
	}
	...
	if ( ! empty( $locations[ $theme_location ] ) ) {
		...
		if ( empty( get_post_meta( $menu_id, 'user_menu_id', true ) ) ) {
