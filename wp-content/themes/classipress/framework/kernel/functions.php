...
function appthemes_get_template_directories() {
	...
	/**
	 * Filters the registered template directories
	 *
	 * First two directories refer to the stylesheet directory and the template
	 * directory.
	 *
...
function appthemes_pagenavi( $wp_query = null, $query_var = 'paged', $args = array() ) {
	...
	/**
	 * Filters the AppThemes pagination arguments.
	 *
	 * @see paginate_links()
	 *
	 * @param array $args An array of arguments to be passed to paginate_links().
	 */
 ...
 *
 * @uses appthemes_get_template_directories() to get all registered template locations
 * @link https://core.trac.wordpress.org/attachment/ticket/18302/18302.2.2.patch
 * @param string|array $template_names Template file(s) to search for, in order.
 *
...
function appthemes_categories_list( $args, $terms_args = array() ) {
	...
	$defaults = array(
		...
		'taxonomy' => 'category',
		// HTML wrappers.
		'column_wrapper'    => '<div class="catcol%2$s">%1$s' . "</div><!-- /catcol -->\r\n",
		'main_list_wrapper' => '<ul class="maincat-list">%s' . "</ul>\r\n",
		'main_item_wrapper' => '<li class="maincat cat-item-%1$d"><a href="%2$s" title="%3$s">%4$s</a> %5$s %6$s' . "</li>\r\n",
		'sub_list_wrapper'  => '<ul class="subcat-list">%s' . "</ul>\r\n",
		'sub_item_wrapper'  => '<li class="cat-item cat-item-%1$d"><a href="%2$s" title="%3$s">%4$s</a> %5$s %6$s' . "</li>\r\n",
		'count_wrapper'     => '<span class="cat-item-count">(%d)</span>',
		'nocats_wrapper'    => '<ul class="subcat-list"><li class="cat-item">%s' . "</li>\r\n</ul>\r\n",
	);
	...
	foreach ( $terms as $key => $value ) {
		if ( $value->parent == $terms_args['child_of'] || ( empty( $terms_args['child_of'] ) && ! in_array( $value->parent, $term_ids, true ) ) ) {
			$cats[ $key ] = $terms[ $key ];
		} else {
	...
	foreach ( $cats as $cat ) {
		...
		$show_count = $options['cat_parent_count'] ? sprintf( $options['count_wrapper'], $cat->count ) : '';

		if ( $options['menu_sub_num'] > 0 ) {
			// create child tree
			$temp_menu = appthemes_create_child_list( $subcats, $options['taxonomy'], $cat->term_id, 0, $options['menu_depth'], $options['menu_sub_num'], $options['cat_child_count'], $options['cat_hide_empty'], array(
				'sub_list_wrapper'  => $options['sub_list_wrapper'],
				'sub_item_wrapper'  => $options['sub_item_wrapper'],
				'count_wrapper'     => $options['count_wrapper'],
			) );

			if ( ! $temp_menu && ! $options['cat_nocatstext'] ) {
				$temp_menu = sprintf( $options['nocats_wrapper'], __( 'No categories', APP_TD ) );
			}
		}

		if ( is_callable( $options['main_item_wrapper'] ) ) {
			$cat_html .= call_user_func_array( $options['main_item_wrapper'], array( $cat, $show_count, $temp_menu, $options ) );
		} else {
			$cat_html .= sprintf( $options['main_item_wrapper'], $cat->term_id, get_term_link( $cat, $options['taxonomy'] ), esc_attr( $cat->description ), $cat->name, $show_count, $temp_menu );
		}

		$i++;

		if ( ( $i % $cats_per_col ) == 0 || $i >= $total_main_cats ) {
			$cat_menu .= sprintf( $options['column_wrapper'], sprintf( $options['main_list_wrapper'], $cat_html ), $first );
		}
 ...
 * @return string|bool
 */
function appthemes_create_child_list( $subcats = array(), $taxonomy = 'category', $parent = 0, $curr_depth = 0, $max_depth = 3, $max_subcats = 3, $child_count = true, $hide_empty = false, $wrappers = array() ) {
	$child_menu = '';
	$curr_subcats = 0;
	...
	foreach ( $subcats as $subcat ) {
		if ( $subcat->parent == $parent ) {
			...
			// only show the total count if option is set
			$show_count = $child_count ? sprintf( $wrappers['count_wrapper'], $subcat->count ) : '';

			$temp_menu = appthemes_create_child_list( $subcats, $taxonomy, $subcat->term_id, $curr_depth, $max_depth, $max_subcats, $child_count, $hide_empty, $wrappers );
			if ( ! $temp_menu ) {
				$temp_menu = '';
			}
			$child_menu .= sprintf( $wrappers['sub_item_wrapper'], $subcat->term_id, get_term_link( $subcat, $taxonomy ), esc_attr( $subcat->description ), $subcat->name, $show_count, $temp_menu );
		}
	}

	if ( ! empty( $child_menu ) ) {
		return sprintf( $wrappers['sub_list_wrapper'], $child_menu );
	} else {
		return false;
	}
}
 ...
 */
function appthemes_get_all_taxonomy( $id = 0, $taxonomy = null, $before = '', $sep = '', $after = '' ) {
	 ...
	 *
	 * The dynamic portion of the filter name refers to the taxonomy name.
	 *
	 * @param array $term_links An array of all taxonomy terms links.
	 */
...
function appthemes_get_edit_post_url( $post_id = 0, $context = 'display' ) {
	...
	/**
	 * Filters the Edit Post form URL.
	 *
	 * @param string $url     The form URL.
	 * @param int    $post_id The post ID.
	 * @param string $context The context of using.
	 */
...
function appthemes_get_edit_term_url( $term_id, $taxonomy, $object_type = '' ) {
	...
	/**
	 * Filters the Edit Term form URL.
	 *
	 * @param string $url         The form URL.
...
function appthemes_get_edit_comment_url( $comment_id = 0, $context = 'display' ) {
	...
	/**
	 * Filters the Edit Comment form URL.
	 *
	 * @param string $url        The form URL.
