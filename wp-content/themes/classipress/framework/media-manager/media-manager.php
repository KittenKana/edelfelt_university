...
class APP_Media_Manager {
	...
	private function init_hooks() {
		add_action( 'appthemes_media_manager', array( __CLASS__, 'output_hidden_inputs' ), 10, 6 );
	...
	static function output_media_manager( $object_id = 0, $atts = array(), $filters = array() ) {
		...
		if ( empty( $atts['attachment_ids'] ) && $object_id ) {
			...
			$attachment_ids = (array) $meta_object->get_meta( $object_id, $attach_field_id, true );
			...
			if ( ! empty( $attachment_ids ) ) {
				...
				$args = array(
					...
					'meta_query'  => array(
						array(
							'key'   => '_app_attachment_type',
							'value' => appthemes_get_mm_allowed_meta_types( 'file' ),
						),
					),
				);
				$atts['attachment_ids'] = $meta_object->get_object_attachments( $object_id, $args )->posts;
			...
			if ( ! empty( $embeds_attach_ids ) ) {
				...
				$args = array(
					...
					'meta_query'  => array(
						array(
							'key'   => '_app_attachment_type',
							'value' => appthemes_get_mm_allowed_meta_types( 'embed' ),
						),
					),
				);

				$curr_embed_attachments = $meta_object->get_object_attachments( $object_id, $args )->posts;
				...
				if ( ! empty( $curr_embed_attachments ) ) {
					...
					// refresh the post meta
					$meta_object->update_meta( $object_id,  $attach_field_id .'_embeds', $embeds_attach_ids );
				}
		...
		if ( ! is_array( $atts['embed_urls'] ) ) {
			$atts['embed_urls'] = array();
		}
		...
		/**
		 * Fires after Media Manager outputs.
		 *
		 * @param string $attach_field_id The field name.
		 * @param array  $attachment_ids  The file attachments IDs.
		 * @param array  $embed_urls      The embedded media URLs.
		 * @param array  $filters         An array of file filters.
		 * @param string $object          The object type (post, user, etc...).
		 * @param int    $object_id       The object ID.
		 */
		do_action( 'appthemes_media_manager', $attach_field_id, $atts['attachment_ids'], $atts['embed_urls'], $filters, $atts['object'], $object_id );
	}
	...
	static function handle_media_upload( $object_id, $type = 'post', $fields = array(), $duplicate = false ) {
		...
		self::maybe_clear_old_attachments( $object_id, $attachs, false, $meta_object );
	}
	...
	private static function handle_embed_field( $object_id, $field, $type = 'post' ) {
		...
		if ( empty( $_POST[ $field ] ) ) {
			...
			foreach( $embeds as $embed ) {
				...
				$oembed_type = ( ! empty( $oembed->type ) ? $oembed->type : 'unknown' );
				...
				$attachment = array(
					...
					'post_content'   => $embed,
					'guid'           => $embed,
					'post_mime_type' => sprintf( '%1s/%2s', $oembed_type, $iframe_type ),
				);

				// Set post_parent only for objects of 'post' type.
				$post_parent = ( 'post' === $type ) ? $object_id : 0;

				// Assign the embed URL to the object as a normal file attachment.
				$attach_id = wp_insert_attachment( $attachment, '', $post_parent );

				if ( is_wp_error( $attach_id ) ) {
					continue;
				}

				self::set_attachment_parent( $attach_id, $object_id, $meta_object->get_meta_type() );
		...
		/**
		 * Fires after Media Manager handled embeds.
		 *
		 * @param int    $object_id The object ID.
		 * @param string $field     The field name.
		 * @param type   $type      The object type (post, user, etc..).
		 */
	...
	private static function handle_media_field( $object_id, $field, $type = 'post', $duplicate = false ) {
		...
		if ( empty( $_POST[ $field ] ) ) {
			...
			foreach( $attachments as $attachment_id ) {
				...
				$attach_parent_id = $attachment->_app_attachment_parent_id ? $attachment->_app_attachment_parent_id : $attachment->post_parent;

				if ( $attach_parent_id != $object_id ) {
					...
					if ( $duplicate && $attach_parent_id ) {
						$attachment->ID = 0;
						$attachment->post_parent = 0;
						$attachment->post_author = get_current_user_id();
						$generate_meta = true;
					}
					...
					$attach_id = wp_insert_attachment( $attachment, $filename, $post_parent );
					if ( is_wp_error( $attach_id ) ) {
						continue;
					}
				...
				update_post_meta( $attach_id, '_app_attachment_type', $meta_type );
				update_post_meta( $attach_id, '_app_media_manager_parent', $field );
			}
		...
		/**
		 * Fires after Media Manager handled media files.
		 *
		 * @param int    $object_id The object ID.
		 * @param string $field     The field name.
		 * @param type   $type      The object type (post, user, etc..).
	 ...
	 */
	static function output_hidden_inputs( $attach_field_id, $attachment_ids, $embed_urls, $filters, $object_type, $object_id ) {
		...
		// Input for the attachment parent object ID.
		echo html( 'input', array( 'name' => "{$attach_field_id}_object_id", 'type' => 'hidden', 'value' => $object_id ) );
	...
	static function restrict_media_library( $query ) {
		...
		if ( ! empty( $_REQUEST['query']['author'] ) ) {
			$_REQUEST['query']['app_object_id'] = $_REQUEST['query']['author'];
			unset ( $_REQUEST['query']['author'] );
		}
		...
		if ( empty( $_REQUEST['post_id'] ) ) {
			...
			$query['meta_query'][] = array(
				'key'   => '_app_media_manager_parent',
				'value' => $mm_id,
			);

			if ( ! empty( $_REQUEST['query']['app_object_id'] ) ) {
				$query['meta_query'][] = array(
					'relation' => 'OR',
					array(
						'key'   => '_app_attachment_parent_id',
						'value' => absint( $_REQUEST['query']['app_object_id'] ),
					),
					array(
						'key'     => '_app_attachment_parent_id',
						'compare' => 'NOT EXISTS',
	...
	static function validate_upload_restrictions( $file ) {
		...
		$meta_object = APP_Media_Manager_Meta_Type_Object_Factory::get_instance( $options['attributes']['object'] );
		...
		if ( ! empty( $_POST['app_file_limit'] ) ) {
			$object_id   = ! empty( $_POST['app_object_id'] ) ? absint( $_POST['app_object_id'] ) : 0;
			$attachments = $meta_object->get_object_attachments( $object_id, array(
				'author'      => get_current_user_id(),
				'post_status' => 'any',
				'meta_query'  => array(
					// Limit files considering the media manager parent ID since
					// each available media manager on a form can have it's own
					// file limits.
					array(
						'key'   => '_app_media_manager_parent',
						'value' => $mm_id,
	...
	static function get_post_attachments( $object_id, $args = array() ) {
		...
		_deprecated_function( __METHOD__, '', 'APP_Media_Manager_Meta_Type::get_object_attachments()' );
	 ...
	 */
	static function maybe_clear_old_attachments( $object_id, $attachments = array(), $delete = false, $meta_object = null ) {

		if ( ! $meta_object || ! $object_id ) {
			return;
		}
		...
		$args = array(
			...
			'meta_query'  => array(
				array(
					'key'   => '_app_attachment_type',
					'value' => appthemes_get_mm_allowed_meta_types(),
				),
			),
		);
		...
		$old_attachments = $meta_object->get_object_attachments( $object_id, $args )->posts;
		...
		foreach( $old_attachments as $old_attachment ) {
			...
			if ( in_array( $type, appthemes_get_mm_allowed_meta_types('embed') ) || $delete ) {
				...
				self::set_attachment_parent( $old_attachment->ID, 0, $meta_object->get_meta_type() );
			}
		}
	...
	*/
	static function set_attachment_mm_id( $attach_id ) {
		...
		if ( $mm_id ) {
			if ( isset( $_REQUEST['app_object_id'] ) ) {
				$options	 = appthemes_get_media_manager_options( $mm_id );
				$meta_object = APP_Media_Manager_Meta_Type_Object_Factory::get_instance( $options['attributes']['object'] );

				self::set_attachment_parent( $attach_id, absint( $_REQUEST['app_object_id'] ), $meta_object->get_meta_type() );
			}
	 ...
	 */
	static function set_attachment_parent( $attach_id, $parent_id, $parent_type ) {
		if ( 'post' !== $parent_type ) {
			update_post_meta( $attach_id, '_app_attachment_parent_type', $parent_type );
			update_post_meta( $attach_id, '_app_attachment_parent_id', $parent_id );
		}
	}
