...
add_filter( 'manage_media_columns', '_appthemes_manage_media_columns' );
add_filter( 'manage_media_custom_column', '_appthemes_manage_media_custom_column', 10, 2 );
...
function _appthemes_media_capabilities( $caps, $cap, $user_id, $args ) {
	...
	switch( $cap ) {
		...
		case 'upload_files':
			...
			/**
			 * Filters the capability to upload files by a given user.
			 *
			 * @param bool $allow   The capability value. Default false.
 ...
 * Expects the fields index type: 'attachs' or 'embeds' (e.g: $fields = array( 'attachs' => array( 'field1', 'field2' ), 'embeds' => array( 'field1', 'field2' ) )
...
function appthemes_output_attachment( $attachment_id, $show_description = true, $show_image_thumbs = true ) {
	...
	appthemes_load_template( array( 'parts/media-manager-attachment.php', 'media-manager/template/media-manager-attachment.php' ), array(
		'app_attachment_id'     => $attachment_id,
		'app_show_description'  => $show_description,
		'app_show_image_thumbs' => $show_image_thumbs,
	) );
...
function appthemes_get_mime_type_icon_class( $mime_type ) {
	...
	/**
	 * Filters the CSS class of an icon for a mime type.
	 *
...
function appthemes_get_mm_allowed_meta_types( $type = '' ) {
	...
	/**
	 * Filters allowed attachments meta types.
	 *
	 * The dynamic portion of the hook relates to the The attachment type:
	 * 'file' or 'embed', or all types, if empty.
	 * For all type use filter `appthemes_mm_allowed_meta_types`.
	 *
	 * @param array $meta_types The meta types array.
	 */
 ...
 */
function _appthemes_manage_media_columns( $posts_columns ) {
	$posts_columns['media_manager_id'] = _x( 'Media Manager ID', 'column name', APP_TD );
	$posts_columns['media_manager_type'] = _x( 'Parent type', 'column name', APP_TD );
	$posts_columns['media_manager_parent'] = _x( 'Parent object', 'column name', APP_TD );
 ...
 */
function _appthemes_manage_media_custom_column( $column_name, $post_id ) {

	$attachment = get_post( $post_id );

	if ( 'media_manager_id' === $column_name ) {
		echo $attachment->_app_media_manager_parent;
	}

	if ( 'media_manager_type' === $column_name ) {
		echo $attachment->_app_attachment_parent_type ? $attachment->_app_attachment_parent_type : 'post';
	}

	if ( 'media_manager_parent' === $column_name ) {
		echo $attachment->_app_attachment_parent_id ? $attachment->_app_attachment_parent_id : get_post( $post_id )->post_parent;
	}
