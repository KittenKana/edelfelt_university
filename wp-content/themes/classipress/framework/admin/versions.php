...
function appthemes_updated_version_notice() {
	...
	foreach ( $args_sets as $args ) {
		...
		/**
		 * Fires in the updated version notice after upgrade procedure before
		 * saving new version in options.
		 *
		 * The dynamic portion of the hook refers to the version option name.
		 *
		 * The hook here needed to avoid duplicated versions checks in upgrade
		 * procedures.
		 *
		 * @param string $new_version The new version tag.
		 * @param string $old_version The old version tag.
		 */
		...
		/**
		 * Filters the Upgrade notice text.
		 *
