...
class APP_Meta_Box extends scbPostMetabox {
	...
	public function form_fields() {
		 ...
		 * Filters form fields.
		 *
		 * The dynamic portion of the hook name, refers to the meta box ID.
		 *
		 * @param $form_fields An array of form fields to be filtered.
		 */
	...
	public function table_row( $row, $formdata, $errors = array() ) {
		...
		return html( 'tr',
			html( "th $style scope='row'", html( 'label for="'.esc_attr( $row['name'] ).'"', $row['title'] ) . $tip ),
			html( "td $style", $input )
		);
	}
